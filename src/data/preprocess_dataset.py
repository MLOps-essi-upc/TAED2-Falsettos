...
def preprocess_data(data, feature_extractor, total_labels, audio_length):
    ...
    if len(audio_decoded)<audio_length*16000: #audio is shorter than one second, we need to pad it
        audio_decoded = np.pad(audio_decoded, (0, 16000-len(audio_decoded)), 'constant')
    elif len(audio_decoded)>audio_length*16000: #audio is longer than one second, we need to cut it
...
def main():
    ...
    for split, dataset_split in audio_dataset.items():
        ...
        preprocessed_datsplit = [None]*len(dataset_split)
        for i in range(len(dataset_split)):
            preprocessed_datsplit[i] = preprocess_data(dataset_split[i],
                feature_extractor, params["total_labels"], params["audio_length"])
